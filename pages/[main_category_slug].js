          ...
          return (
            <>
              <div className="border-b-2">
                <p className="font-bold text-3xl text-header-color mb-2 mt-10">
                  {item.categoryName}
                </p>
              </div>
                ...
                return (
                  <>
                    <div className="mb-10 mt-10">
                      <p className="font-bold text-2xl text-header-color mt-2">
...
export async function getStaticProps({ params }) {
  const { data } = await client.query({
    query: GET_FıLTER_MAINCATEGORY,
    variables: { main_category_slug: params.main_category_slug.toString() },
  });
  return {
    props: {
